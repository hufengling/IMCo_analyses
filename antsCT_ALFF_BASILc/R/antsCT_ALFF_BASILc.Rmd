...
antsct_subj <- read_table(here("input/csvs/demographics/antsCt_subj.txt"), col_names = FALSE) 
pnc_demographics <- pnc_demographics %>% rename(scanid = "id")
basil_subj <- basil_subj %>% separate(X1, c("scanid", "settings"), sep = "_")
antsct_subj <- antsct_subj %>% separate(X1, c("scanid", "settings"), sep = "_")

t1_qa <- read_csv(here("input/csvs/demographics/n1601_t1QaData_v2.csv"))
t1_qa_filtered <- t1_qa %>% filter(t1Exclude == 0, t1RawDataExclude == 0, t1PostProcessExclude == 0, fsReviewExclude == 0 | is.na(fsReviewExclude), fsFinalExclude == 0)

common_pts <- Reduce(intersect, list(pnc_demographics$scanid, basil_subj$scanid, antsct_subj$scanid, t1_qa_filtered$scanid)) %>%
  as.data.frame() %>%
  rename(subj_num = ".")
...
input_filepaths <- common_pts %>% 
  mutate(modality_1 = paste0(subj_num, "_alffStd.nii.gz") %>% file.path("~/Documents/IMCo_analyses/input/niftis/alff", .), #TOCHANGE
         modality_2 = paste0(subj_num, "_CorticalThicknessNormalizedToTemplate2mm.nii.gz") %>% file.path("~/Documents/IMCo_analyses/input/niftis/antsCt", .),
         modality_3 = paste0(subj_num, "_basil.nii.gz") %>% file.path("~/Documents/IMCo_analyses/input/niftis/cbf-basil", .))
...
rm(pnc_demographics, common_pts, basil_subj, antsct_subj, t1_qa)
```

## Load non-nifti PNC data
```{r}
#TOCHANGE
t1_qa_filtered <- t1_qa_filtered %>% rename(subj_num = scanid)
...
demographics <- right_join(demographics, input_filepaths, 
                           by = "subj_num") %>% 
  left_join(t1_qa_filtered, by = "subj_num") %>% 
...
predictors <- demographics %>%
  select(sex, ageAtScan1, race2, pcaslRelMeanRMSMotion, restRelMeanRMSMotion, averageManualRating)

write_csv(predictors, file.path(here("input/csvs/predictors.csv")))

rm(demographics, t1_qa_filtered)
```
...
modalities <- c("alff", "antsCt", "cbf-basil") #TOCHANGE
fwhm <- 3
prop_miss <- 0.9
...
process_pval_image <- function(file_path, mask, out_dir, pval_name) {
  ...
  pval_numeric_clean[pval_numeric_clean == 1] <- NA
...
convert_to_ants <- function(adjusted_pvals, threshold, mask_indices, reference) {
  ...
  adjusted_pvals[is.na(adjusted_pvals)] <- 0
...
} else if (length(settings$modalities) == 3) {
  file_labels <- c(paste0(settings$modalities[1], "_age"),
                   paste0(settings$modalities[1], "_sex"),
                   paste0(settings$modalities[2], "_age"),
                   paste0(settings$modalities[2], "_sex"),
