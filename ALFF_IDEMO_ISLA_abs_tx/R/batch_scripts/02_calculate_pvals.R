...
transform_voxels <- function (image_vector) {
  vec_norm <- image_vector * 1.5 - 0.5
  return(log(vec_norm) - log(1 - vec_norm))
}
...
analyze_coupled_images <- function(nifti_dir, mask, predictors, cores = 2, is_modality, file_paths = NULL) {
  ...
  if (!is_modality) {
    cat("Transforming voxels\n")
    image_vector_list <- lapply(image_vector_list, transform_voxels)
  }
