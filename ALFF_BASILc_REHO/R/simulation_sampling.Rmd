...
write.csv(alff_cbf_df, 
          file.path(settings$output_dir, "csvs/simulation_sampling.csv"))
...
with(alff_cbf_df, plot(alff_ind, cbf_ind))
with(alff_cbf_df, plot(alff_abs, cbf_abs))

with(alff_cbf_df, plot(pimco, alff_ind))
ggplot(alff_cbf_df) + geom_point(aes(pimco, alff_ind, color = log(cbf_abs + 0.25))) + 
...
with(alff_cbf_df, plot(pimco, alff_abs))
ggplot(alff_cbf_df) + geom_point(aes(pimco, alff_abs, color = sqrt(cbf_abs))) + 
  scale_fill_continuous(breaks = c(0, 2.5, 5)) +
  theme_minimal()
with(alff_cbf_df, plot(pimco, cbf_abs))

with(alff_cbf_df, plot(pimco, max_abs))
...
```{r}
s1 <- ggplot(alff_cbf_df) + geom_point(aes(alff_abs, cbf_abs)) + 
  labs(x = "WLR-ALFF Magnitude", y = "WLR-CBF Magnitude") +
  theme(panel.border = element_blank(), 
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = "gray85"),
        axis.line = element_line(colour = "black"),
...
# s4 <- ggplot(alff_cbf_df) + geom_point(aes(pimco, cbf_ind, color = log(alff_abs + 0.25))) + 
#   labs(x = "pIMCo", y = "WLR-CBF") +
#   theme(panel.border = element_blank(), 
#         panel.background = element_blank(),
#         panel.grid.major.y = element_line(color = "gray85"),
#         axis.line = element_line(colour = "black"),
...
#         axis.title = element_text(size = xy_title_size),
#         plot.title = element_text(size = title_size, hjust = 0.5),
#         legend.position = "none"); s4

s2 <- ggplot(alff_cbf_df) + geom_point(aes(pimco, alff_abs, color = log(cbf_abs + 0.25))) + 
  labs(x = "pIMCo", y = "WLR-ALFF Magnitude") +
  theme(panel.border = element_blank(), 
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = "gray85"),
        axis.line = element_line(colour = "black"),
...
s3 <- ggplot(alff_cbf_df) + geom_point(aes(pimco, cbf_abs, color = log(alff_abs + 0.25))) + 
  labs(x = "pIMCo", y = "WLR-CBF Magnitude") +
  theme(panel.border = element_blank(), 
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = "gray85"),
        axis.line = element_line(colour = "black"),
...
kendall_cor <- cor(alff_cbf_df[, c(1, 4, 5)], method = "kendall")
rownames(kendall_cor) <- c("pIMCo", "WLR-CBF \nMagnitude", "WLR-ALFF \nMagnitude")
colnames(kendall_cor) <- rownames(kendall_cor)
s4 <- ggcorrplot(kendall_cor, type = "upper", 
                 show.diag = TRUE, lab = TRUE,
                 ggtheme = theme_classic(),
                 show.legend = FALSE,
                 colors = c("#D55E00", "white", "#0072B2")); s4

supplement_figure <- ggarrange(s1, s2, s3, s4, nrow = 2, ncol = 2); supplement_figure

ggsave(filename = here("output/figures/simulation/supplement_pimco_imco.png"), plot = supplement_figure, type = "cairo-png", width = 15.083, height = 10.906, dpi = 300)
```


```{r}
m1 <- ggplot(alff_cbf_df) + geom_point(aes(alff_ind, cbf_ind)) + 
  labs(x = "WLR-ALFF", y = "WLR-CBF") +
  theme(panel.border = element_blank(), 
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = "gray85"),
        axis.line = element_line(colour = "black"),
...
m2 <- ggplot(alff_cbf_df) + geom_point(aes(pimco, alff_ind, color = log(cbf_abs + 0.25))) + 
  labs(x = "pIMCo", y = "WLR-ALFF") +
  theme(panel.border = element_blank(), 
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = "gray85"),
        axis.line = element_line(colour = "black"),
...
m3 <- ggplot(alff_cbf_df) + geom_point(aes(pimco, max_abs)) + 
  labs(x = "pIMCo", y = "Max WLR Magnitude") +
  theme(panel.border = element_blank(), 
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = "gray85"),
        axis.line = element_line(colour = "black"),
...
kendall_cor <- cor(alff_cbf_df[, c(1, 3, 6)], method = "kendall")
rownames(kendall_cor) <- c("pIMCo", "WLR-ALFF", "Max WLR \n Magnitude")
colnames(kendall_cor) <- rownames(kendall_cor)
m4 <- ggcorrplot(kendall_cor, type = "lower", 
                 show.diag = TRUE, lab = TRUE,
                 ggtheme = theme_classic(),
                 show.legend = FALSE,
...
```{r}
ggarrange(s1, s2, m3, s4, nrow = 2, ncol = 2)
```
