...
input_filepaths <- common_pts %>% 
  mutate(modality_1 = paste0(subj_num, "_pvgmpnc_cbf.nii.gz") %>% file.path("~/Documents/IMCo_analyses/input/niftis/cbf-basil", .), 
         modality_2 = paste0(subj_num, "_sigchange_cope1_TaskStd.nii.gz") %>% file.path("~/Documents/IMCo_analyses/input/niftis/idemo", .))
...
modalities <- c("cbf-basil", "idemo") #TOCHANGE
fwhm <- 3
prop_miss <- 0.9
...
fs_images_dir <- list(file.path(settings$output_dir, "../../ALFF_BASIL/output/freesurfer_images/MNI_space/descriptive"),
                      ...
                      file.path(settings$output_dir, "../../ALFF_BASIL_IDEMO/output/freesurfer_images/MNI_space/descriptive"))
...
```{r}
coupling_dir <- list(file.path(settings$output_dir, "niftis/coupled/global_wcov"),
                     file.path(settings$nifti_dir, "alff"),
                     file.path(settings$nifti_dir, "cbf-basil"),
                     file.path(settings$nifti_dir, "idemo")
                     )
for(dir in coupling_dir){
  print(dir)
  for (file in list.files(dir)[1:5]) {
    image <- check_ants(file.path(dir, file))
    n <- length(which(image == 0))
    print(n)
  }
